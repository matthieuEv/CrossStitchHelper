...
- [x] Détection des tracés de point arrière : un trait qui **traverse plusieurs cases** (contrairement à un symbole de point plein, toujours contenu dans une seule case, et au quadrillage imprimé, toujours aligné sur les axes — voir `backend/app/type_bc.py::_is_grid_ruling`, généralisé dans `backend/app/grid_lines.py` plutôt que dupliqué)
- [x] Détection des points de nœud : petite forme isolée, pas alignée sur le pavage régulier des cases coloriées
- [x] Distinction entre point arrière **décoratif** (texte ou bordure d'une page de garde/légende, hors de la grille de travail) et point arrière **réel** (fait partie du motif, doit être signalé au fil) — ne jamais extraire le premier comme s'il fallait le broder
- [x] Type A : croiser avec les tableaux de légende déjà présents mais ignorés depuis le Lot 4 ("Floss Used for Backstitch"/"French Knots", distincts de "Full Stitches") pour valider les comptages, comme déjà fait pour les points entiers (§7.3)
- [x] Vérifier explicitement (fixtures existantes, pas seulement de nouvelles) que cette extraction ne dégrade jamais la reconnaissance des points pleins déjà en place (Lots 4-5) — un point arrière qui traverse une case de la grille de symboles type B/C ne doit jamais faire basculer cette case en case incertaine à tort
...
**Lot 9 terminé.** Troisième et dernière étape du moteur d'extraction type A (`backend/app/type_a.py`) : les cinq sections de légende ("Full/Half/Quarter Stitches", "French Knots", "Back Stitches") sont désormais toutes parsées, pas seulement la première — une couleur déclarée dans plusieurs sections (ex. DMC 742 : point entier + point arrière + nœud) garde un seul index de palette. Rapprochement couleur→DMC par **couleur exacte** de l'échantillon vectoriel de la légende (mesuré identique au tracé du motif, RVB à RVB — un repli par distance Lab existe mais s'est révélé mesurablement peu fiable : confusions 938/3031, jamais d'assignation à 310) plutôt que par police de symboles, ces deux sections n'en ayant pas. Distinction point arrière/réglure de quadrillage généralisée dans `backend/app/grid_lines.py` (extrait de `type_bc.py::_is_grid_ruling`, comportement Lot 5 préservé bit à bit sans critère de longueur) : un point arrière part lui aussi d'un coin de case comme une réglure, seules la longueur et l'orientation (diagonale, jamais possible pour une réglure) les séparent réellement. Un point arrière est tracé deux fois dans cette fixture (passe foncée + passe plus claire) : dédoublonné. Chaque page de grille porte aussi des teintes délavées de duplication de bande de recouvrement avec la page voisine — jamais comptées, la correspondance par couleur exacte les exclut naturellement puisqu'elles ne correspondent à aucun échantillon de légende. Points 1/4 (4 cas, DMC 3031) non distinguables par glyphe+couleur de fond (identiques à ceux du point entier) : seule leur position décentrée dans la case (mesurée, jamais un pavage supposé) les repère. Longueur de point arrière convertie en cm uniquement quand l'utilisateur a renseigné un compte de toile à la validation (`ImportRecapForm`) — jamais une conversion inventée sans lui ; le compte de toile lu dans le texte du PDF ("Fabric: Aida 16") pré-remplit ce champ sans jamais l'imposer (`ImportConfig.detected_fabric_count`, `frontend/src/screens/ImportScreen.tsx`). Aucune UI de correction manuelle pour ces quatre couches détectées (contrairement à `detected_cells`+`fills` depuis le Lot 2) : hors périmètre de ce lot, qui ne couvre que la détection — une correction manuelle après import reste à faire directement en case à cocher/décocher depuis le suivi (Lot 8). Vérifié contre la fixture de référence (`backend/tests/test_type_a.py`, valeurs reparsées indépendamment de la page 11 "Usage Summary" — y compris la colonne "Back(cm)", en réalité en **pouces**, mesuré à moins de 0,1 % d'écart sur les 8 codes) et contre les cinq autres fixtures (aucune régression, `backend/tests/test_type_bc.py`/`test_type_e.py`/`test_grid_lines.py`, 82 tests d'extraction). Travail délégué à l'agent `pdf-extraction-specialist` pour le moteur d'extraction proprement dit (`type_a.py`, `grid_lines.py`), vérifié indépendamment (diff relu en entier, `ruff`/`mypy`/six fixtures re-mesurées) ; le branchement dans le pipeline de validation (`backend/app/api/imports.py::commit`, `ImportConfig`/`ImportConfigPatch`) et le pré-remplissage du compte de toile côté assistant ont été faits séparément, vérifiés par un test e2e bout en bout (`frontend/e2e/lot9-special-stitches-extraction.spec.ts`) contre la fixture réelle.
