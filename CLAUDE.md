...
**Lot 8 terminé** (« Finitions » — quatre sous-chantiers assez indépendants livrés en PR séparées, décision du 18/09/2026 ; le partage communautaire des recettes en a été retiré, l'utilisateur n'en a pas l'usage). Points fractionnés et spéciaux **terminés** : le modèle de progression suit maintenant cinq catégories (point entier, 1/2, 1/4, point arrière, nœud) au lieu d'une seule, chacune avec son propre bitmap et son propre espace d'index (`backend/app/models.py::Progress`) — le pourcentage global d'avancement reste basé sur le point entier uniquement. Rendu canvas et interaction tactile ajoutés (`frontend/src/pattern/render.ts`, `state/useTracker.ts`) : le point arrière/nœuds reste **visible à tout niveau de zoom** (corrigé le 20/09/2026 après retour d'usage réel — comme sur un diagramme papier), seule l'interaction (cocher) restant réservée au même seuil de zoom que l'apparition des symboles. Sauvegarde/restauration des données **terminée** : un document JSON unique (`backend/app/backup.py`, `GET`/`POST /api/backup*`) couvre toute l'instance — motifs, palette, grille, progression (les cinq catégories ci-dessus comprises), journal d'activité et recettes — jamais `ImportJob` (transitoire) ni `AppMeta` (bookkeeping interne). Une restauration est un remplacement complet, jamais une fusion ; « Effacer toutes les données » (réglages) réutilise ce même mécanisme avec un document vide. Sauvegarde automatique quotidienne en tâche de fond du processus (`backend/app/auto_backup.py`, aucune dépendance d'infrastructure supplémentaire), activable depuis les réglages. Thème sombre **terminé** : l'infrastructure existait déjà (portée depuis les maquettes) mais était incomplète — audit case par case plutôt que supposée correcte, trois bugs réels corrigés (`frontend/src/index.css`) et l'éclair de mauvais thème au chargement éliminé par un script bloquant dans `frontend/index.html`. Traductions FR/EN **terminées** : le garde-fou de compilation entre `fr.ts`/`en.ts` garantit déjà qu'aucune clé ne manque, l'audit a donc porté sur ce qu'il ne peut pas voir — locale `Intl` suivant le navigateur plutôt que la langue choisie (`StatsScreen.tsx`, corrigé), et surtout **tous** les messages d'erreur de l'API et **tous** les avertissements de détection automatique (type_a/bc/e) qui étaient des chaînes françaises figées côté serveur : restructurés en `code` + `params` (`backend/app/schemas.py::ApiErrorDetail`/`DetectionWarning`), traduits côté client (`frontend/src/lib/api.ts`). La restructuration du moteur d'extraction a été déléguée puis vérifiée indépendamment (diff relu en entier, tests propres, six fixtures réelles re-mesurées identiques avant/après).
